...
public class BestLocationListener extends Observable implements LocationListener,GpsStatus.NmeaListener,Listener {
    ...
    private LocationManager o_locationManager;
    private String gps_profider_name;
    private String last_gprmc = "";
    private long mLastLocationMillis;
    private boolean isGPSFix = false;
    private gps_test bean = new gps_test();
    ...
    public void onProviderDisabled(String provider) {
        // do nothing.
        Log.d("gps_debug : " ,"provider disable");
    }

    @Override
    public void onProviderEnabled(String provider) {
        // do nothing.
        Log.d("gps_debug : ", "provider enable");
    }

    @Override
    public void onStatusChanged(String provider, int status, Bundle extras) {
        // do nothing.
//        Log.d("gps_debug : " ,"StatusChanged");
//        for (String p : extras.keySet()){
//            Log.d("gps_debug extras : " ,p + " " + extras.get(p));
//        }
    }
...
//        Log.d("gps_accuracy", ""+locationIsMostAccurate);
        if (DEBUG) {
...
//            onBestLocationChanged(location);
            mLastLocation = location;
            GpsClock.getInstance().setTime(bean.getGps_d());
...
//            onBestLocationChanged(location);
    ...
    public void register(LocationManager locationManager, boolean gps) {
        ...
        for (int i = 0; i < providersCount; i++) {
            ...
            gps_profider_name = providerName;
            System.out.println("register gps: " + LocationManager.GPS_PROVIDER + " --------- " + providerName);
    ...
    public void onGpsStatusChanged(int event) {
        ...
        switch (event) {
            ...
            case GPS_EVENT_STARTED:
                Log.d("gps_debug2: " , ""+ GPS_EVENT_STARTED);
                break;
            case GPS_EVENT_SATELLITE_STATUS:
//                int satellitenumber = 0;
//                float signalintensity = 0;
//                int statenumber = 0;
//                int count = 0;
//                int maxSatellites = currentgpsstatus.getMaxSatellites();
//                Iterator<GpsSatellite> iters = currentgpsstatus.getSatellites().iterator();
//                while (iters.hasNext() && count <= maxSatellites) {
//                    GpsSatellite s = iters.next();
//                    statenumber = s.getPrn();
//                    signalintensity = s.getSnr();
//                    if (signalintensity >= 22){
//                        satellitenumber++;
//                    }
//                    //updateWithNewLocation(location);
//                    Log.d("gps_nmea", "sate: " + (count + 1) + "single: "
//                            + signalintensity + "num:" + statenumber);
//                    count++;
//                }
//                bean.setSat_num(satellitenumber);
//                if (DEBUG)


                if (last_gprmc.length()>0){
                    gprmc = last_gprmc.split(",");
//                    Log.d("gps_nmea: " , "len: " + gprmc.length + " - " + last_gprmc);
//                    DebugParam.getInstance().addMessage(last_gprmc);
                    if (gprmc.length == 13){
                        bean.setVelocity(gprmc[7]);
                    }
                }
...
//                Log.d("gps_debug3: " , "sate_int: " + satellitenumber);
                break;
            case GPS_EVENT_STOPPED:
                bean.setIs_valid(false);
                Log.d("gps_debug4: " , ""+ GPS_EVENT_STOPPED);
                break;
        }
    }
}
